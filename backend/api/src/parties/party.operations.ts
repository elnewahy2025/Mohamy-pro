...
import { PartyAccessDeniedError } from './party.errors';

export const PARTY_PERMISSION: PermissionKey =
...
export class PartyOperations {
  private readonly logger = new Logger(PartyOperations.name);
  ...
  async authorize(request: Request): Promise<PartyContext> {
    const auth = request.auth;
    if (!auth) throw new PartyAccessDeniedError('UNAUTHENTICATED');
    if (!auth.activeTenantId)
      throw new PartyAccessDeniedError('TENANT_CONTEXT_REQUIRED');
    const { membershipId: actorMembershipId } =
      await this.permissions.assertTenantPermission({
        request,
        userId: auth.userId,
        tenantId: auth.activeTenantId,
        permissionKey: PARTY_PERMISSION,
  ...
  async run<T>(
    ...
    try {
      ...
      if (error instanceof PartyAccessDeniedError) throw error;
      this.logger.warn({
  ...
  async requirePartyInTenant(
    transaction: Prisma.TransactionClient,
    ctx: PartyContext,
    partyId: string,
  ): Promise<void> {
    const found = await transaction.party.findFirst({
      where: { id: partyId, tenantId: ctx.tenantId },
      select: { id: true },
    });
    if (!found) throw new PartyAccessDeniedError('NO_PARTY_IN_TENANT');
  }
