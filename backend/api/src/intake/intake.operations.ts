...
import { IntakeAccessDeniedError } from './intake.errors';

export const INTAKE_PERMISSION: PermissionKey =
...
@Injectable()
export class IntakeOperations {
  private readonly logger = new Logger(IntakeOperations.name);
  ...
  async authorize(request: Request): Promise<IntakeContext> {
    const auth = request.auth;
    if (!auth) throw new IntakeAccessDeniedError('UNAUTHENTICATED');
    if (!auth.activeTenantId)
      throw new IntakeAccessDeniedError('TENANT_CONTEXT_REQUIRED');
    const { membershipId: actorMembershipId } =
      await this.permissions.assertTenantPermission({
        request,
        userId: auth.userId,
        tenantId: auth.activeTenantId,
        permissionKey: INTAKE_PERMISSION,
  ...
  async run<T>(
    ...
    try {
      ...
      if (error instanceof IntakeAccessDeniedError) throw error;
      this.logger.warn({
        message: 'Intake operation failed',
