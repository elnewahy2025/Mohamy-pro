...
import { ComplianceAccessDeniedError } from './compliance.errors';
...
export class ComplianceOperations {
  private readonly logger = new Logger(ComplianceOperations.name);
  ...
  async authorize(request: Request): Promise<ComplianceContext> {
    const auth = request.auth;
    if (!auth) throw new ComplianceAccessDeniedError('UNAUTHENTICATED');
    if (!auth.activeTenantId)
      throw new ComplianceAccessDeniedError('TENANT_CONTEXT_REQUIRED');
    const { membershipId: actorMembershipId } =
      await this.permissions.assertTenantPermission({
        request,
        userId: auth.userId,
        tenantId: auth.activeTenantId,
        permissionKey: PERMISSION_KEYS.CAN_MANAGE_COMPLIANCE,
  ...
  async run<T>(
    ...
    try {
          ...
          await this.audit.write(
            {
              ...
              targetId:
                entity && typeof entity === 'object'
                  ? ((entity as { id?: string }).id ?? null)
                  : null,
              policy: PERMISSION_KEYS.CAN_MANAGE_COMPLIANCE,
              correlationId,
      ...
      if (error instanceof ComplianceAccessDeniedError) throw error;
      this.logger.warn({
        message: 'Compliance operation failed',
