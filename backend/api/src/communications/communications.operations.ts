...
import { CommunicationsAccessDeniedError } from './communications.errors';

export const COMMUNICATIONS_PERMISSION: PermissionKey =
...
@Injectable()
export class CommunicationsOperations {
  private readonly logger = new Logger(CommunicationsOperations.name);
  ...
  async authorize(request: Request): Promise<CommunicationsContext> {
    const auth = request.auth;
    if (!auth) throw new CommunicationsAccessDeniedError('UNAUTHENTICATED');
    if (!auth.activeTenantId)
      throw new CommunicationsAccessDeniedError('TENANT_CONTEXT_REQUIRED');
    const { membershipId: actorMembershipId } =
      await this.permissions.assertTenantPermission({
        request,
        userId: auth.userId,
        tenantId: auth.activeTenantId,
        permissionKey: COMMUNICATIONS_PERMISSION,
  ...
  async run<T>(
    ...
    try {
          ...
          await this.audit.write(
            {
              ...
              policy: COMMUNICATIONS_PERMISSION,
              correlationId,
      ...
      if (error instanceof CommunicationsAccessDeniedError) throw error;
      this.logger.warn({
        message: 'Communications operation failed',
