...
import { OpsAccessDeniedError } from './ops.errors';
...
export class OpsOperations {
  private readonly logger = new Logger(OpsOperations.name);
  ...
  async authorize(request: Request): Promise<OpsContext> {
    const auth = request.auth;
    if (!auth) throw new OpsAccessDeniedError('UNAUTHENTICATED');
    if (!auth.activeTenantId)
      throw new OpsAccessDeniedError('TENANT_CONTEXT_REQUIRED');
    const { membershipId: actorMembershipId } =
      await this.permissions.assertTenantPermission({
        request,
        userId: auth.userId,
        tenantId: auth.activeTenantId,
        permissionKey: PERMISSION_KEYS.CAN_MANAGE_OPERATIONS,
  ...
  async run<T>(
    ...
    try {
          ...
          await this.audit.write(
            {
              ...
              policy: PERMISSION_KEYS.CAN_MANAGE_OPERATIONS,
              correlationId,
      ...
      if (error instanceof OpsAccessDeniedError) throw error;
      this.logger.warn({
        message: 'Operations action failed',
