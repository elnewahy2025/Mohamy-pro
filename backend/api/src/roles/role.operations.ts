...
import { RoleAccessDeniedError } from './role.errors';

export const ROLE_MANAGE_PERMISSION: PermissionKey =
...
export class RoleOperations {
  private readonly logger = new Logger(RoleOperations.name);
  ...
  async authorize(request: Request): Promise<RoleContext> {
    const auth = request.auth;
    if (!auth) throw new RoleAccessDeniedError('UNAUTHENTICATED');
    if (!auth.activeTenantId)
      throw new RoleAccessDeniedError('TENANT_CONTEXT_REQUIRED');
    const { membershipId: actorMembershipId } =
      await this.permissions.assertTenantPermission({
        request,
        userId: auth.userId,
        tenantId: auth.activeTenantId,
        permissionKey: ROLE_MANAGE_PERMISSION,
  ...
  async run<T>(
    ...
    try {
      ...
      if (error instanceof RoleAccessDeniedError) throw error;
      this.logger.warn({
