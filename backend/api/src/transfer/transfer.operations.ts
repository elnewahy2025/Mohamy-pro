...
import { TransferAccessDeniedError } from './transfer.errors';

export const TRANSFER_IMPORT_PERMISSION: PermissionKey =
  PERMISSION_KEYS.CAN_MANAGE_IMPORTS;
export const TRANSFER_EXPORT_PERMISSION: PermissionKey =
...
export class TransferOperations {
  private readonly logger = new Logger(TransferOperations.name);
  ...
  async authorize(
    request: Request,
    permissionKey: PermissionKey = TRANSFER_IMPORT_PERMISSION,
  ): Promise<TransferContext> {
    const auth = request.auth;
    if (!auth) throw new TransferAccessDeniedError('UNAUTHENTICATED');
    if (!auth.activeTenantId)
      throw new TransferAccessDeniedError('TENANT_CONTEXT_REQUIRED');
  ...
  async run<T>(
    ...
    policy: PermissionKey = TRANSFER_IMPORT_PERMISSION,
  ): Promise<T> {
    ...
    try {
      ...
      if (error instanceof TransferAccessDeniedError) throw error;
      this.logger.warn({
