...
import { CalendarAccessDeniedError } from './calendar.errors';

export const CALENDAR_PERMISSION: PermissionKey =
...
export class CalendarOperations {
  private readonly logger = new Logger(CalendarOperations.name);
  ...
  async authorize(request: Request): Promise<CalendarContext> {
    const auth = request.auth;
    if (!auth) throw new CalendarAccessDeniedError('UNAUTHENTICATED');
    if (!auth.activeTenantId)
      throw new CalendarAccessDeniedError('TENANT_CONTEXT_REQUIRED');
    const { membershipId: actorMembershipId } =
      await this.permissions.assertTenantPermission({
        request,
        userId: auth.userId,
        tenantId: auth.activeTenantId,
        permissionKey: CALENDAR_PERMISSION,
  ...
  async run<T>(
    ...
    try {
      ...
      if (error instanceof CalendarAccessDeniedError) throw error;
      this.logger.warn({
