...
function makeOps(
  ...
  const service = new ClientAddressService(ops);
...
describe('ClientAddressService', () => {
  it('authorizes and creates an address with the correct event type', async () => {
  ...
  it('removes an address via the removed event type', async () => {
    ...
    await service.remove(request(), 'a1', 'wrong type');
    expect(run).toHaveBeenCalledWith(
      expect.anything(),
      CTX,
