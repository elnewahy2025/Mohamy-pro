...
import { z } from 'zod';
import { ApiError, LegalConfigClient, type CountryResult } from '@/lib/api';
...
export function CountrySection(): React.ReactNode {
  ...
  async function runCreate(form: CountryForm): Promise<void> {
    ...
    setSubmitError(null);
    try {
      const next = await client.createCountry({
        code: form.code,
        name: form.name,
      });
      setCreated(next);
    ...
    <form className="settings-card" noValidate>
      ...
      <div className="form-grid">
        <FormField
          ...
          error={
            errors.code ? t(`form.errors.${errors.code.message}`) : undefined
          }
          inputProps={{
            type: 'text',
            autoComplete: 'off',
        ...
        <div className="operation-result-details" style={{ marginTop: '1rem' }}>
          ...
          {list.map((item: CountryResult) => (
            ...
            >
              <span>{item.name}</span>
              <span>
                <code>{item.code}</code> · {item.status} ·{' '}
                <code>{item.id}</code>
              </span>
            </div>
          ))}
