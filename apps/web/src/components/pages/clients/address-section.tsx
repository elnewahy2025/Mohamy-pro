...
export function AddressSection({
  ...
  const [items, setItems] = useState<ClientAddressResult[]>([]);
  ...
  async function runList(clientId: string): Promise<void> {
    try {
      setItems(await client.listAddresses(clientId));
    } catch {
      setItems([]);
    }
  }
            ...
            <li key={item.id} className="text-sm">
              <Button
                ...
                onClick={() => {
                  ...
                  setValue('line1', item.line1);
                  setValue('line2', item.line2 ?? '');
              ...
              >
                {item.type} — {item.line1.slice(0, 40)}, {item.city}
                {item.isPrimary ? ' ★' : ''}
              </Button>
            </li>
          ))}
