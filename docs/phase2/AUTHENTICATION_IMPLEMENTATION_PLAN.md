...
| Session refresh | `POST /api/v1/auth/refresh` requires the session cookie, approved `Origin`, and matching CSRF token; it uses the encrypted provider refresh token only on the server, atomically replaces it when the provider rotates it, and returns no token material. A refresh failure revokes the application session, clears the cookie, and returns an authentication-required result. |
