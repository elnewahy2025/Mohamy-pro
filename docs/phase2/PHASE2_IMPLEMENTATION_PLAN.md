...
The first implementation slice is recorded in [`RLS_TENANT_CONTEXT_IMPLEMENTATION.md`](RLS_TENANT_CONTEXT_IMPLEMENTATION.md). It adds the transaction-local Prisma helper, pre-membership scope clearing, and a fail-closed RLS migration for the Phase 2 hierarchy, membership, invitation, tenant-role, role-assignment, and denial tables. Static validation, unit tests, and the real Windows disposable PostgreSQL runtime gate are verified for that staged scope. The connected Phase 2 mutation path now uses server-derived membership context, scoped idempotency, append-only audit persistence, and transactional outbox linkage; `StorageObject` remains outside this tenant-switch slice until its own callers are tenant-aware. No permissive `tenantId IS NULL` policy is allowed. The full Phase 2 workstream remains open for the remaining legacy-table boundaries, authorization, abuse-control, generated-client, frontend, and production-topology gates.

### 6. API and frontend contracts

Define Phase 2 REST endpoints under `/api/v1` for identity, membership, tenant selection, and administration. Every endpoint must have OpenAPI documentation, standard success/error envelopes, correlation IDs, validation schemas, authorization policies, and contract tests. The connected tenant-switch mutation now has the frozen envelope, correlation, validation, and idempotency path; generated-client re-entry and real HTTP/database evidence remain required before the surface is accepted as complete.
...
Define and persist append-only audit events for login, logout, session revocation, membership creation/change/removal, role and permission changes, denial decisions, tenant switching, and privileged access. Redact secrets and avoid storing raw tokens or unnecessary personal payloads. The connected audit foundation includes retention/legal-hold controls, immutability triggers, indexes, bounded metrics, and transactional outbox linkage; membership and worker runtime evidence remains pending. The tenant-switch mutation provides the API-originated API-to-outbox-to-worker trace path, which must be exercised against real PostgreSQL and Redis before acceptance.
