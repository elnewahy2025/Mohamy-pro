...
The static and unit results do not prove PostgreSQL behavior. The first Windows attempt reached the runner but stopped at the child-process boundary with `spawnSync pnpm.cmd EINVAL`; this was a Windows process-launch defect, not an RLS result. The Windows-safe runner then successfully applied all six repository migrations and reported `rls_metadata_status=PASS|tables=11|policies=12`, but its disposable Tenant fixture omitted the required `updatedAt` value and stopped before the behavioral checks. After that correction, the next run reached the Membership seed and found the same required `updatedAt` field omitted there. After both fixture corrections, the verifier reached its first behavioral check and returned two organizations without context. That result exposed a verifier-design defect: the configured database account can bypass RLS, so an owner/superuser connection cannot prove default deny. The verifier was changed to create and use a generated `LOGIN NOSUPERUSER NOBYPASSRLS` role with scoped grants before running behavioral checks. The first run of that hardened verifier then exposed a separate safe-identifier defect: the quoting guard rejected the repository’s mixed-case table names before the behavioral checks. The quoting guard was corrected to accept only PostgreSQL identifier characters while supporting both mixed-case fixed table names and generated lowercase names. The next non-bypass run proved default deny and organization isolation, then exposed the actual remaining verifier bug: Role and RolePermission queries were placed after the tenant transaction had committed, so PostgreSQL had correctly cleared the transaction-local tenant settings. Those queries are now inside the active tenant transaction, with an explicit active-context assertion immediately before them. The following run passed through hierarchy integrity and then stopped at the rollback cleanup assertion, which accepted only `NULL` even though a cleared custom setting can be represented as an empty string. The cleanup check now normalizes empty settings with `NULLIF` and separately verifies that post-rollback no-context organization visibility is zero. These fixture and verifier defects did not change the existing `mohamy_pro` data. The runtime acceptance remains open until the corrected non-bypass run provides direct evidence for default deny, malformed context, Tenant A/Tenant B isolation, hierarchy foreign-key rejection, rollback, and reuse of the same pooled connection after transaction-local settings have cleared.
