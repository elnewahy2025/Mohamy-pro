...
The static and unit results do not prove PostgreSQL behavior. Runtime acceptance still requires direct evidence for default deny, malformed context, Tenant A/Tenant B isolation, hierarchy foreign-key rejection, rollback, and reuse of the same pooled connection after transaction-local settings have cleared.
