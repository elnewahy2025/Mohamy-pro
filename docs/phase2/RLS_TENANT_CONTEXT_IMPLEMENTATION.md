...
The static and unit results do not prove PostgreSQL behavior. The first Windows attempt reached the runner but stopped at the child-process boundary with `spawnSync pnpm.cmd EINVAL`; this was a Windows process-launch defect, not an RLS result. The runner now invokes the fixed `pnpm` command through the Windows command shell, with the shell option limited to that fixed executable and argument list. The correction has passed sandbox syntax, formatting, build, unit, lint, and whitespace gates, but the runtime acceptance remains open until the corrected Windows run provides direct evidence for default deny, malformed context, Tenant A/Tenant B isolation, hierarchy foreign-key rejection, rollback, and reuse of the same pooled connection after transaction-local settings have cleared.
...
The verifier creates a generated database whose name begins with `mohamy_phase2_rls_fresh_`, applies the complete repository migration directory, runs the RLS checks, and drops only that generated database. It expects eleven RLS-protected tables and twelve policies because `Membership` has both the tenant-isolation policy and the read-only pre-switch policy. It refuses to target `mohamy_pro`. A successful run must include the following result markers:
