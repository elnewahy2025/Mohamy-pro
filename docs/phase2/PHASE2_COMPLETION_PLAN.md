...
|---|---|---|
| W1 | Contract and e2e tests | **CLOSED.** A dedicated contract suite (`test/contract.e2e-spec.ts`, `test/contract/*`) plus `test/openapi.e2e-spec.ts` assert the frozen success/error envelope shapes, correlation-ID presence + echo, idempotency replay/conflict/concurrency, validation (`forbidNonWhitelisted` → `VALIDATION_FAILED`), and 5xx collapse on the representative business routes. The full-`AppModule` e2e (`app.e2e-spec.ts`) is gated by the pre-existing openid-client ESM jest limitation; the contract/spec surface is exercised without importing `AuthModule`. |
| W2 | OpenAPI fidelity | **CLOSED.** Phase 2 business controllers/DTOs (auth, bootstrap, health) declare `@ApiProperty`/`@ApiBody`/`@ApiResponse`/`@ApiOperation`; `api/docs-json` includes DTO schemas and per-route responses (validated by `openapi.e2e-spec.ts`). |
...
| W6 | Bilingual frontend identity/membership flows | **CLOSED.** `src/proxy.ts` renamed to `src/middleware.ts` so next-intl locale detection is wired (identical content). Added the four identity/membership UI surfaces (bootstrap, tenant-switch, invitation create + accept, membership-admin suspend/expire/remove/reinstate) as `[locale]/identity/*` routes, plus full en+ar message keys and accessible form errors (`aria-invalid`, `aria-describedby`, `role="alert"`) via a reusable `FormField` and an `aria-live` `OperationResult`. The hand-rolled `ApiClient` was extended (typed request/response interfaces matched to the frozen backend DTOs, CSRF + `Idempotency-Key` headers, error-envelope mapping via `ApiError`) and covered by 5 new unit tests. **Generated-client re-entry gate satisfied via the explicit documented-retention path**: owner chose to record `docs/phase2/GENERATED_CLIENT_RETENTION_DECISION.md` (generator selection, versioning, ownership, CI regeneration check, runtime consumer test, compatibility policy, and a concrete regeneration trigger) instead of installing a codegen toolchain. Web `tsc --noEmit` clean (classic TypeScript 5.9.3; the `latest` TypeScript-Go preview `typescript@7.0.2` is broken in this container — pre-existing). Web vitest suite: 13/13 green. |
| W7 | API-level cross-tenant isolation e2e | **DEFERRED with rationale (owner-approved, 2026-09-02).** Cross-tenant isolation is proven at the DB/RLS level (`rls_runtime_result=PASS`; Tenant A `sees_a=1 sees_b=0`). The only HTTP vectors today are membership-admin and tenant-switch; there is no general tenant-scoped business-data surface to drive a meaningful cross-tenant HTTP e2e. Re-entry is gated on the phase introducing the first tenant-scoped data endpoint (or on the required RLS follow-on for StorageObject/OutboxMessage/IdempotencyKeys). See §93. |
...
**Status: CLOSED.**
- Middleware: `src/proxy.ts` → `src/middleware.ts` (git rename, identical content); locale detection now wired via next-intl middleware auto-load.
- Surfaces (all under `[locale]/identity/*`, thin route wrappers + client page components): `/bootstrap`, `/tenant-switch`, `/invitations/create`, `/invitations/accept`, `/members`. Reusable `FormField` supplies `aria-invalid` + `aria-describedby` + `role="alert"` error messages; `OperationResult` renders success/error in an `aria-live="polite"` region. Direction is handled by the existing `dir={ltr|rtl}` html attribute and logical CSS; native inputs/buttons give keyboard navigation.
- Message keys: `identity.{bootstrap,tenantSwitch,invitation,membershipAdmin,result}` + `form.{required,errors}` added to `messages/en.json` and `messages/ar.json` (structurally identical; parity test extended).
- Client: `src/lib/api.ts` extended with typed `bootstrap`, `tenantSwitch`, `createInvitation`, `acceptInvitation`, `suspendMembership`, `expireMembership`, `removeMembership`, `reinstateMembership`; each sends `X-CSRF-Token` + UUIDv4 `Idempotency-Key` (required by the global `IdempotencyInterceptor`) and maps the frozen error envelope to `ApiError`. Added `EN`/`ApiError` types. 5 new unit tests in `src/lib/api.test.ts`.
- Generated-client gate: explicit documented retention via `GENERATED_CLIENT_RETENTION_DECISION.md` (owner-selected path) — records generator (`openapi-typescript`), versioning, ownership, CI regeneration check, runtime consumer test, compatibility policy, and a concrete regeneration trigger.
- No frontend authorization decisions: no permission gating in the UI; the backend enforces all authz/MFA/tenant boundaries and returns error envelopes the client surfaces.
- **Evidence:** web `tsc --noEmit` 0 errors (TypeScript 5.9.3 classic; `typescript@latest` = broken tsgo preview `7.0.2` in this container — pre-existing env limitation); web vitest 13/13 green (api 10, messages 3).

### W7 — API-level cross-tenant isolation e2e (DEFERRED with rationale, not silent)
- **Intended exit gate:** an HTTP e2e asserting Tenant A cannot read/modify Tenant B data through the API (complementing the DB-level RLS check).
- **Disposition (owner-approved, 2026-09-02):** recorded as deferred, not silently dropped. Rationale:
  - DB/RLS-level cross-tenant isolation is already proven and evidenced (`rls_runtime_result=PASS`, `tenant_a_sees_a=1 tenant_a_sees_b=0`). No HTTP e2e can add isolation proof beyond the RLS boundary the requests must traverse.
  - There is **no general tenant-scoped business-data endpoint** in Phase 2. The only tenant-scoped HTTP surface is membership-admin (`membership/members/*`) and tenant-switch, both operating on the `Membership` table through `withTenantContext`. A cross-tenant HTTP assertion therefore exercises only that slice, not a business-data surface.
  - `RLS_TENANT_CONTEXT_IMPLEMENTATION.md` explicitly marks the RLS boundary as **staged**: StorageObject, OutboxMessage, and IdempotencyKey remain out of enforcement, and API/worker caller integration is not yet claimed. Closing a general HTTP isolation e2e before those follow-ons would overstate the boundary.
  - The jest e2e harness cannot load the full `AppModule` (openid-client ESM), and driving membership-admin requires synthesized session + CSRF + recent-MFA state plus a live RLS Postgres — prerequisites not currently available.
- **Re-entry condition:** reintroduce the HTTP cross-tenant isolation e2e when a tenant-scoped business-data endpoint (first real legal/org surface) is added, and when the RLS follow-on tables are within enforcement. Membership-slice isolation remains covered by unit/spec and the staged RLS runtime verification.
...
| Tenant isolation (API) | RLS check remains PASS (`rls_runtime_result=PASS`). HTTP cross-tenant isolation e2e deferred to the phase introducing a tenant-scoped business-data endpoint (see W7 §95-99); membership-slice isolation is spec-covered. |
...
9. [`GENERATED_CLIENT_RETENTION_DECISION.md`](GENERATED_CLIENT_RETENTION_DECISION.md)
