...
| W3 | Named-policy RBAC matrix | **PARTIAL.** Only `CanManageMembership` is enforced (invitation + membership-admin). `CanSwitchTenant` and `CanReadOrganizationSettings` are docs-only, absent from source. No route asserts `CanViewTenant`/`CanInviteMembers`/`CanManageRoles`/`CanCreateTenant`/`CanGrantPlatformAdmin`. Tenant-switch is not gated by a named policy. |
| W4 | Failed-auth lockout + MFA-failure limit | **DORMANT SECURITY GAP.** `registerAuthenticationFailure`, `checkLockout`, `enforceMfaFailure`, and `ACCOUNT_LOCKED`/`ACCOUNT_LOCK_RELEASED` events are implemented but have **zero call sites**; lockout is unreachable. |
| W5 | Auth-lifecycle + role audit events | **INCOMPLETE.** `auth.login.started/succeeded/denied`, `auth.logout`, `role.assigned` are declared but never emitted. Allowlist typo: `audit-event.service` allows `'auth.login.start'` while the declared type is `'auth.login.started'`. |
