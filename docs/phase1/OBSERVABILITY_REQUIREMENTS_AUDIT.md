...
| Application and worker error metrics | HTTP filter, queue service, and outbox worker record bounded fixed-category errors. | Windows API scrape registered both families; the real worker job completed, but the API registry cannot expose worker-local samples. The dedicated worker metrics endpoint is now implemented and requires a fresh Windows scrape. | `PARTIALLY VERIFIED` | Scrape `http://localhost:3002/metrics` after a worker job and capture incremented worker histogram/error samples. |
...
| Windows API runtime and scrape | Windows repository root | PASS; API and worker started, readiness returned HTTP 200, `/api/metrics` returned HTTP 200 with Prometheus content, required metric families were captured, and the real outbox success path reached `PROCESSED`. The worker-local metrics endpoint was not present in that older runtime and requires a fresh run after `8174559f`. |
