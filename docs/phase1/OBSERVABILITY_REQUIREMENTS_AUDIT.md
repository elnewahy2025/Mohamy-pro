...
| Application and worker error metrics | HTTP filter, queue service, and outbox worker record bounded fixed-category errors. | Windows API scrape registered both families; a fresh worker scrape at `http://localhost:3002/metrics` returned HTTP 200 and captured `mohamy_worker_job_duration_seconds_count{job_name="outbox.dispatch"} 1`. No error occurred during the controlled success workflow. | `PASS WITH SCOPE LIMIT` | Retain hosted scrape and alerting evidence; execute controlled error increments only in an isolated failure test. |
